...
LLM : Ollama via LangChain ReAct (langgraph.prebuilt)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# AGENT SETUP  (Ollama + langgraph — same pattern as simple_ollama_agent.py)
# ─────────────────────────────────────────────────────────────────────────────

from langchain_ollama import ChatOllama
from langgraph.prebuilt import create_react_agent  # ✅ only working import in your env
...
# ── LLM: Ollama (replaces ChatGoogleGenerativeAI) ────────────────────────────
llm = ChatOllama(
    model=model,
    temperature=0.3,
...
# ── Agent: langgraph create_react_agent (no hub.pull, no AgentExecutor) ──────
agent = create_react_agent(
    llm,
    ALL_TOOLS,
    prompt=SYSTEM_PROMPT,   # system prompt passed directly
)
...
╚══════════════════════════════════════════════════════════════════╝
  ...
  LLM Engine : Ollama / {model} (ReAct Agent via langgraph)
